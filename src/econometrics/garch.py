"""
Q-RiskNet India — GARCH Volatility Estimation Module
...
def estimate_garch_volatility(returns_series):
    """
    Estimates conditional volatility series using an asymmetric GJR-GARCH(1,1,1) model.
    """
    sector_name = returns_series.name
    with diag.DiagnosticTimer(f"GJR-GARCH(1,1,1) Volatility Proxy Estimation for {sector_name}"):
        try:
            ...
            diag.log_warning(f"GJR-GARCH failed to converge for {sector_name}: {e}. Trying standard GARCH(1,1)...")
            try:
                ...
                diag.log_warning(f"Standard GARCH failed for {sector_name}. Fallback to 10-day rolling std dev.")
